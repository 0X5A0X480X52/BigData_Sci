...
from .service_bridge import EvidenceRAGServiceBridge
from .tools import TOOL_DEFINITIONS, get_all_tool_schemas


class EvidenceRAGMCPServer(BaseMCPServer):
    """MCP server for evidence-rag tools."""
    ...
    def __init__(self, bridge: EvidenceRAGServiceBridge) -> None:
        self.bridge = bridge

    def call(self, name: str, **kwargs: Any) -> Any:
        if name not in self.tool_names:
            raise ValueError(f"Unknown evidence-rag tool: {name}")
...
def main() -> None:
    ...
    service = EvidenceRAGService(artifact_store, config)
